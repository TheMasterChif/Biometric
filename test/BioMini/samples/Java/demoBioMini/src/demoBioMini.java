...
public class demoBioMini extends JFrame {
	...
	private JCheckBox jCheckBox_en_detect_fake_advanced = null;
	 ...
	 public void UpdateScannerList()
	{
	    ...
	    for(int j=0;j<nNumber;j++){
			...
			if(nRes == 0 && hScanner!=null){
				...
				if(nRes == 0) nRes = p.UFS_GetScannerType(hScanner[0], Type);
                
                if(nRes == 0) {
                    String szId = new String(bScannerId);
                    szListLog =  "ID : " + szId + "Type : " + Type[0];
	...
	public int testCallStartCapturing()
	{
		...
		if(hScanner!=null){
			...
			setStatusMsg("get Scanner handle success pointer:"+hScanner[0]);
		}else{
...
//			getParameter
	...
	public int CaptureSingle() 
	{
		...
		if(hScanner!=null){
			System.out.println("GetScannerHandle return hScanner pointer: " + hScanner[0]);
			setStatusMsg("get Scanner handle success pointer:" + hScanner[0]);
		}else{
			...
			jButton_Uninit.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(nRes ==0){
						...
						nInitFlag=0;
						if(nRes == 0) MsgBox("UFS_Uninit success!");
					}else{
	...
	private JCheckBox getJCheckBox_en_detect_fake_advanced() {
		if (jCheckBox_en_detect_fake_advanced == null) {
			jCheckBox_en_detect_fake_advanced = new JCheckBox();
			jCheckBox_en_detect_fake_advanced.setBounds(new Rectangle(220, 220, 60, 18));
			jCheckBox_en_detect_fake_advanced.setText("Ex");
			
			jCheckBox_en_detect_fake_advanced.addActionListener(new java.awt.event.ActionListener() {
				@Override
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(hScanner!=null){
						...
						if(jCheckBox_en_detect_fake_advanced.isSelected()){
							pValue[0] = 1;
							int nRes =  p.UFS_SetParameter(hScanner[0],p.UFS_PARAM_LFD_TYPE,pValue);
							if(nRes==0){
								setStatusMsg("Setting UFS_PARAM_LFD_TYPE = "+pValue[0]+" successful.");
							}
							else{
								setStatusMsg("Setting UFS_PARAM_LFD_TYPE failed : "+nRes);
							}
						}
						else{
							pValue[0] = 0;
						...
						setStatusMsg("getCurrentScannerHandle fail!! in ChangeCheckBox-en_detect_fake_advanced");
					}
				}
			});
		}
		return jCheckBox_en_detect_fake_advanced;
	}
			...
			jButton_enroll.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(hScanner!=null){
						...
						if(nRes == 0) nRes = p.UFS_CaptureSingleImage(hScanner[0]);
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.add(getJCheckBox_en_detect_fake_advanced(), null);
