...
namespace Suprema {
	public partial class Biomini_Demo : Form {
		//=========================================================================//
		...
		private void Biomini_Demo_Load(object sender, EventArgs e) {
			int i;
            ...
            m_pUFTemplateBuf = new byte[MAX_TEMPLATE_SIZE * 20];
            Array.Clear(m_pUFTemplateBuf, 0, m_pUFTemplateBuf.Length);
	        m_pUFTemplateBufSize = new int[20];
	        for(i=0; i<20; i++) m_pUFTemplateBufSize[i] = 0;

            m_pISOTemplateBuf = new byte[MAX_TEMPLATE_SIZE * 20];
            Array.Clear(m_pISOTemplateBuf, 0, m_pISOTemplateBuf.Length);
		...
		private void Biomini_Demo_FormClosing(object sender, FormClosingEventArgs e) {
            UFScanner Scanner;
            UFS_STATUS ufs_res;
            ...
            {
                tbxMessage.AppendText("UFScanner RemoveScannerCallback: OK\r\n");
            }
            else
            {
                UFScanner.GetErrorString(ufs_res, out m_strError);
                tbxMessage.AppendText("UFScanner RemoveScannerCallback: " + m_strError + "\r\n");
            }
		...
		static void Main()
		{
			...
			Application.Run(new Biomini_Demo());
		}
		//=========================================================================//
		...
		private void GetCurrentScannerSettings()
		{
            ...
            cbFakeLevel.SelectedIndex = Scanner.DetectFake;
		}
		...
		private void btnEnroll_Click(object sender, EventArgs e) {
            ...
            if (cbEnrollUI.Checked)
            {
                bool bHasData = false;

                // to set template size
                Scanner.TemplateSize = MAX_TEMPLATE_SIZE;
                ...
                if (sizeOfUFBufs >= MAX_TEMPLATE_SIZE * 20 || sizeOfISOBufs >= MAX_TEMPLATE_SIZE * 20)
                {
                    bHasData = false;
                    Array.Clear(m_pUFTemplateBufSize, 0, m_pUFTemplateBufSize.Length);
                    Array.Clear(m_pISOTemplateBufSize, 0, m_pISOTemplateBufSize.Length);
                }

                int nOptions;
                Scanner.PackOptions(
                    cbSecurityLevel.SelectedIndex, 
                    m_quality,
                    nFingersToEnroll,
                    nTemplatesPerFinger,
                    out nOptions);

                byte[] pUFBuf = new byte[MAX_TEMPLATE_SIZE * 20];
                int[] pUFBufSize = new int[20];
                byte[] pISOBuf = new byte[MAX_TEMPLATE_SIZE * 20];
                int[] pISOBufSize = new int[20];

                Array.Clear(pUFBuf, 0, pUFBuf.Length);
                Array.Clear(pUFBufSize, 0, pUFBufSize.Length);
                Array.Clear(pISOBuf, 0, pISOBuf.Length);
                Array.Clear(pISOBufSize, 0, pISOBufSize.Length);

                if (bHasData)
                {
                    m_pUFTemplateBuf.CopyTo(pUFBuf, 0);
                    m_pUFTemplateBufSize.CopyTo(pUFBufSize, 0);
                    m_pISOTemplateBuf.CopyTo(pISOBuf, 0);
                    m_pISOTemplateBufSize.CopyTo(pISOBufSize, 0);
                }

                UFS_STATUS nRet = Scanner.EnrollUI(cbTimeout.SelectedIndex, nOptions, pUFBuf, pUFBufSize, pISOBuf, pISOBufSize, null);
                ...
                pUFBuf.CopyTo(m_pUFTemplateBuf, 0);
                pUFBufSize.CopyTo(m_pUFTemplateBufSize, 0);
                pISOBuf.CopyTo(m_pISOTemplateBuf, 0);
                pISOBufSize.CopyTo(m_pISOTemplateBufSize, 0);

                if (nRet == UFS_STATUS.OK || nRet == UFS_STATUS.ERROR) return;
            }
            ...
            Biomini_UserInfo udlg = new Biomini_UserInfo();
            ...
            //Enroll with Advanced-Extraction
                ...
                Biomini_Enroll dlg = new Biomini_Enroll();
		...
		private void btnVerify_Click(object sender, EventArgs e) {
            ...
            if (cbEnrollUI.Checked)
            {
                ...
                int nFingerIndex = -1;
                UFS_STATUS nRet = UFS_STATUS.ERROR;
                switch (m_nType)
                {
                    case 0:
                        nRet = Scanner.VerifyUI(cbTimeout.SelectedIndex, nOptions, 2001, m_pUFTemplateBuf, m_pUFTemplateBufSize, null, out nFingerIndex);
                        break;
                    case 1:
                        nRet = Scanner.VerifyUI(cbTimeout.SelectedIndex, nOptions, 2002, m_pISOTemplateBuf, m_pISOTemplateBufSize, null, out nFingerIndex);
                        break;
                }
        ...
        private void cbLanguage_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Scanner.Language = cbLanguage.SelectedItem.ToString();
        }

        private void cbFakeLevel_SelectedIndexChanged(object sender, EventArgs e)
        {
            UFScanner Scanner;
            if (!GetGetCurrentScanner(out Scanner))
            {
            ...
            Scanner.DetectFake = cbFakeLevel.SelectedIndex;
        }

        private void cbEnDetectFakeAdvanced_CheckedChanged(object sender, EventArgs e)
        {
            UFScanner Scanner;
            int Ex_checked = 0;
            if (!GetGetCurrentScanner(out Scanner))
            ...
            if (cbEnDetectFakeAdvanced.Checked == true) Ex_checked = 1;
            else Ex_checked = 0;

            Scanner.LFDType = Ex_checked;
        }
