// Biomini_DemoDlg.cpp : implementation file
//

#include "stdafx.h"
#include "Biomini_Demo.h"
#include "Biomini_DemoDlg.h"
#include "Biomini_EnrollDlg.h"
...
CBiomini_DemoDlg::CBiomini_DemoDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CBiomini_DemoDlg::IDD, pParent)
	, m_ctl_useEnrollUI(FALSE)
	, m_ctl_strLanguage(_T(""))
	, m_ctl_enableDetectFakeEx(FALSE)
{
	//{{AFX_DATA_INIT(CBiomini_DemoDlg)
	m_nBrightness = -1;
	m_nSensitivity = -1;
	m_bDetectCore = FALSE;
	m_nSecurityLevel = 2;
...
void CBiomini_DemoDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBiomini_DemoDlg)
	...
	DDX_Check(pDX, IDC_USE_ENROLLUI, m_ctl_useEnrollUI);
	DDX_CBString(pDX, IDC_LANG_ENROLLUI, m_ctl_strLanguage);
	DDX_CBIndex(pDX, IDC_CB_FAKELEVEL, m_nFakeLevel);
	DDX_Check(pDX, IDC_EN_DETECT_FAKE_ADVANCED, m_ctl_enableDetectFakeEx);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CBiomini_DemoDlg, CDialog)
	//{{AFX_MSG_MAP(CBiomini_DemoDlg)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_DESTROY()
	...
	ON_CBN_SELCHANGE(IDC_LANG_ENROLLUI, OnSelchangeLanguage)
	//}}AFX_MSG_MAP
	ON_CBN_SELCHANGE(IDC_CB_FAKELEVEL, &CBiomini_DemoDlg::OnCbnSelchangeCbFakelevel)
	ON_BN_CLICKED(IDC_EN_DETECT_FAKE_ADVANCED, &CBiomini_DemoDlg::OnBnClickedEnDetectFakeAdvanced)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CBiomini_DemoDlg message handlers

BOOL CBiomini_DemoDlg::OnInitDialog()
{
	...
	m_pUFTemplateBuf = new unsigned char[1024*20];
	memset(m_pUFTemplateBuf, 0, 1024*20);
	m_pUFTemplateBufSize = new int[20];
	for(i=0; i<20; i++) m_pUFTemplateBufSize[i] = 0;

	m_pISOTemplateBuf = new unsigned char[1024*20];
	memset(m_pISOTemplateBuf, 0, 1024*20);
	m_pISOTemplateBufSize = new int[20];
	for(i=0; i<20; i++) m_pISOTemplateBufSize[i] = 0;
...
void CBiomini_DemoDlg::OnDestroy() 
{
	CDialog::OnDestroy();
	
	int ufs_res = UFS_RemoveScannerCallback();
	if (ufs_res != UFS_OK) {
		UFS_GetErrorString(ufs_res, m_strError);
		AddMessage("UFS_RemoveScannerCallback: %s\r\n", m_strError);
	}
	...
	delete[] m_pUFTemplateBuf;
	delete[] m_pUFTemplateBufSize;
	delete[] m_pISOTemplateBuf;
	delete[] m_pISOTemplateBufSize;
}
...
HCURSOR CBiomini_DemoDlg::OnQueryDragIcon()
{
...
/////////////////////////////////////////////////////////////////////////////
void CBiomini_DemoDlg::AddMessage(char* szData, ...)
{
	#define MAX_BUF 1024
...
void CBiomini_DemoDlg::OnClearMessage() 
{
	m_message.Empty();
	m_ctlMessageEdit.SetWindowText(m_message);
}

void CBiomini_DemoDlg::GetScannerTypeString(int nScannerType, char* strScannerType)
{
	switch (nScannerType) {
	...
	case UFS_SCANNER_TYPE_SFR300v2:
		...
		break;
	case UFS_SCANNER_TYPE_SFR410:
		sprintf(strScannerType, "SFR410");
		break;
	case UFS_SCANNER_TYPE_SFR500:
		sprintf(strScannerType, "BioMini Plus");
		break;
	case UFS_SCANNER_TYPE_SFR550:
		sprintf(strScannerType, "BioMini Plus2");
		break;
	case UFS_SCANNER_TYPE_SFR600:
		sprintf(strScannerType, "BioMini Slim");
		break;
	case UFS_SCANNER_TYPE_SFR700:
		sprintf(strScannerType, "BioMini Slim 2");
		break;
	default:
...
BOOL CBiomini_DemoDlg::GetCurrentScannerHandle(HUFScanner* phScanner)
{
...
void CBiomini_DemoDlg::GetCurrentScannerSettings()
{
	HUFScanner hScanner;
	int value;
	...
	m_nFakeLevel = value;
	((CComboBox*)GetDlgItem(IDC_CB_FAKELEVEL))->SetCurSel(m_nFakeLevel);

	UpdateData(FALSE);
}

void CBiomini_DemoDlg::GetMatcherSettings(HUFMatcher hMatcher)
{
	int value;
...
void CBiomini_DemoDlg::OnSelchangeScannerList() 
{
	GetCurrentScannerSettings();
}

void CBiomini_DemoDlg::AddRow(int nSerial, char* szUserID, BOOL bTemplate1, BOOL bTemplate2)
{
	int nIndex;
	char str_tmp[256];
...
void CBiomini_DemoDlg::UpdateFingerDataList()
{
	int i;
...
void CBiomini_DemoDlg::OnSelchangeLanguage()
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
	CString strPath;
	...
	strPath = m_ctl_strLanguage;

	ufs_res = UFS_SetParameter(hScanner, UFS_PARAM_LANGUAGE, (void *)(LPCTSTR)strPath);
	if (ufs_res != UFS_OK) {
		UFS_GetErrorString(ufs_res, m_strError);
		AddMessage("UFS_SetParameter(UFS_LANGUAGE): %s\r\n", m_strError);
	}

	return;
}

void CBiomini_DemoDlg::OnSelchangeTimeout() 
{
    if (m_nTimeout == -1) {
		return;
	}
...
void CBiomini_DemoDlg::OnCbnSelchangeCbFakelevel()
{
	if (m_nFakeLevel == -1) {
		return;
	}
	...
	// Unit of timeout is millisecond
	value = m_nFakeLevel;
	ufs_res = UFS_SetParameter(hScanner, UFS_PARAM_DETECT_FAKE, &value);
	if (ufs_res != UFS_OK) {
		UFS_GetErrorString(ufs_res, m_strError);
		AddMessage("UFS_SetParameter(UFS_PARAM_DETECT_FAKE): %s\r\n", m_strError);
	}
}

void CBiomini_DemoDlg::OnChangeBrightness() 
{
    if (m_nBrightness == -1) {
		return;
	}
...
void CBiomini_DemoDlg::OnChangeSensitivity() 
{
    if (m_nSensitivity == -1) {
		return;
	}
...
void CBiomini_DemoDlg::OnDetectCore() 
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
	int value;
...
void CBiomini_DemoDlg::OnSelchangeEnrollQuality() 
{
    UpdateData(TRUE);
}

void CBiomini_DemoDlg::OnSelchangeId() 
{
    UpdateData(TRUE);
}

void CBiomini_DemoDlg::OnSelchangeSecurityLevel() 
{
	UFM_STATUS ufm_res;
	int value;
...
void CBiomini_DemoDlg::OnFastMode() 
{
	UFM_STATUS ufm_res;
	int value;
...
/////////////////////////////////////////////////////////////////////////////
void CBiomini_DemoDlg::UpdateScannerList()
{
	UFS_STATUS ufs_res;
	int nScannerNumber;
	int i;
...
LRESULT CBiomini_DemoDlg::OnUpdateScannerList(WPARAM wParam, LPARAM lParam)
{
	UpdateScannerList();
...
void CBiomini_DemoDlg::OnUpdate() 
{
	UFS_STATUS ufs_res;
...
void CBiomini_DemoDlg::OnUninit() 
{
	/////////////////////////////////////////////////////////////////////////////
...
{
	CBiomini_DemoDlg* dlg = (CBiomini_DemoDlg*)pParam;
...
void CBiomini_DemoDlg::OnStartCapturing() 
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
...
void CBiomini_DemoDlg::OnAutoCapture() 
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
...
void CBiomini_DemoDlg::OnAbortCapturing() 
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
...
void CBiomini_DemoDlg::OnCaptureSingle() 
{
	HUFScanner hScanner;
	UFS_STATUS ufs_res;
...
void CBiomini_DemoDlg::OnExtract() 
{
	UpdateData(TRUE);
	...
	// Check encrypt / decrypt function
	{
		...
		int bCheckTemp = 0;
		ufm_res = UFM_Verify(m_hMatcher, Template, TemplateSize, Tmp2, Tmp2Size, &bCheckTemp);
		if(!bCheckTemp) {
			UFM_GetErrorString(ufm_res, m_strError);
			AddMessage("UFS_Encrypt/Decrypt is failed: %s\r\n", m_strError);
		}
	}
	*/
...
void CBiomini_DemoDlg::OnEnroll() 
{
	UpdateData(TRUE);
	...
	const int nFingersToEnroll = 10;
	const int nTemplatesPerFinger = 2;
	int nTemplateSize = 384;

	if (!GetCurrentScannerHandle(&hScanner)) {
		return;
	}

	if( m_ctl_useEnrollUI ){
		bool bHasData = false;
		GetDlgItem(IDC_ENROLL)->SetWindowTextA("Enroll");

		UFS_SetParameter(hScanner, UFS_PARAM_TEMPLATE_SIZE, &nTemplateSize);

		int sizeOfUFBufs = 0, sizeOfISOBufs = 0;
		for(int i=0; i<20; i++) {
			sizeOfUFBufs += m_pUFTemplateBufSize[i];
			sizeOfISOBufs += m_pISOTemplateBufSize[i];
			if(m_pUFTemplateBufSize[i] != 0 && m_pISOTemplateBufSize[i] != 0) {
				bHasData = true;
			}
		}
		if(sizeOfUFBufs >= nTemplateSize*20 || sizeOfISOBufs >= nTemplateSize*20) {
			bHasData = false;
			memset(m_pUFTemplateBufSize, 0, 20*sizeof(int));
			memset(m_pISOTemplateBufSize, 0, 20*sizeof(int));
		}

		int nOptions =
			UF_PACK_SECURITY(m_nSecurityLevel+1) |
			UF_PACK_QUALITY(m_quality) |
			UF_PACK_NFINGERS(nFingersToEnroll) |
			UF_PACK_NTEMPLATES(nTemplatesPerFinger);

		BYTE* pUFBuf = new BYTE[nTemplateSize*20];
		int* pUFBufSize = new int[20];
		BYTE* pISOBuf = new BYTE[nTemplateSize*20];
		int* pISOBufSize = new int[20];

		memset(pUFBuf, 0, nTemplateSize*20*sizeof(BYTE));
		memset(pUFBufSize, 0, 20*sizeof(int));
		memset(pISOBuf, 0, nTemplateSize*20*sizeof(BYTE));
		memset(pISOBufSize, 0, 20*sizeof(int));

		if(bHasData) {

			memcpy(pUFBuf, m_pUFTemplateBuf, nTemplateSize*20*sizeof(BYTE));
			memcpy(pUFBufSize, m_pUFTemplateBufSize, 20*sizeof(int));
			memcpy(pISOBuf, m_pISOTemplateBuf, nTemplateSize*20*sizeof(BYTE));
			memcpy(pISOBufSize, m_pISOTemplateBufSize, 20*sizeof(int));
			GetDlgItem(IDC_ENROLL)->SetWindowTextA("Modify");
			UpdateData(FALSE);

		}

		UFS_STATUS nRet = UFS_EnrollUI(hScanner, m_nTimeout, nOptions, pUFBuf, pUFBufSize, pISOBuf, pISOBufSize, NULL);
		if(nRet == UFS_OK) GetDlgItem(IDC_ENROLL)->SetWindowTextA("Modify");

		memset(m_pUFTemplateBuf, 0, nTemplateSize*20*sizeof(BYTE));
		memset(m_pUFTemplateBufSize, 0, 20*sizeof(int));
		memset(m_pISOTemplateBuf, 0, nTemplateSize*20*sizeof(BYTE));
		memset(m_pISOTemplateBufSize, 0, 20*sizeof(int));

		memcpy(m_pUFTemplateBuf, pUFBuf, nTemplateSize*20*sizeof(BYTE));
		memcpy(m_pUFTemplateBufSize, pUFBufSize, 20*sizeof(int));
		memcpy(m_pISOTemplateBuf, pISOBuf, nTemplateSize*20*sizeof(BYTE));
		memcpy(m_pISOTemplateBufSize, pISOBufSize, 20*sizeof(int));
		...
		if(nRet == UFS_OK || nRet == UFS_ERROR) return;
	}
	...
	CBiomini_EnrollDlg dlg;
	CBiomini_UserInfoDlg udlg;
...
void CBiomini_DemoDlg::OnVerify() 
{
	UpdateData(TRUE);
	...
	if( m_ctl_useEnrollUI ){
		...
		switch (m_nType) {
		case 0:
			nRet = UFS_VerifyUI(hScanner, m_nTimeout, nOptions, UFS_TEMPLATE_TYPE_SUPREMA, m_pUFTemplateBuf, m_pUFTemplateBufSize, NULL, &nFingerIndex);
			break;
		case 1:
			nRet = UFS_VerifyUI(hScanner, m_nTimeout, nOptions, UFS_TEMPLATE_TYPE_SUPREMA, m_pISOTemplateBuf, m_pISOTemplateBufSize, NULL, &nFingerIndex);
			break;
		}
...
void CBiomini_DemoDlg::OnIdentify() 
{
	UpdateData(TRUE);
	...
	/*/
	// 2. Using UFM_IdentifyInit and UFM_IdentifyNext
	{
		int i;
		...
		for (i = 0; i < m_template_num; i++) {
			...
			ufm_res = UFM_IdentifyNext(m_hMatcher, template_all[i], templateSize_all[i], &bIdentifySucceed);
			if (ufm_res != UFM_OK) {
				UFM_GetErrorString(ufm_res, m_strError);
...
void CBiomini_DemoDlg::OnSaveTemplate() 
{
	int nSelected;
...
void CBiomini_DemoDlg::OnSaveImage() 
{
	int nFileType =0;
	CFileDialog dlg(FALSE, "wsq", NULL, NULL, "Bitmap Files (*.bmp)|*.bmp|WSQ Files (*.wsq)|*.wsq||");
	//CFileDialog dlg(FALSE, "bmp", NULL, NULL, "Bitmap Files (*.bmp)|*.bmp");
	if (dlg.DoModal() != IDOK) {
		return;
	}else{
		nFileType = dlg.m_ofn.nFilterIndex ;
	}
	...
	}else if( nFileType ==2){ // wsq
		ufs_res = UFS_SaveCaptureImageBufferToWSQ(hScanner, 0.75, (char*)(LPCTSTR)dlg.GetPathName());
		if (ufs_res == UFS_OK) {
			AddMessage("Captured image is saved\r\n");
		}
	}
...
void CBiomini_DemoDlg::OnDeleteAllTemplate() 
{
	int i;
	for (i = 0; i < MAX_TEMPLATE_NUM; i++) {
		memset(m_template1[i], 0, MAX_TEMPLATE_SIZE);
...
void CBiomini_DemoDlg::OnUpdateTemplate() 
{
	UpdateData(TRUE);
...
void CBiomini_DemoDlg::OnDeleteTemplate() 
{
	int nSelected;
	int i;
...
void CBiomini_DemoDlg::OnSelchangeQuality() 
{
	UpdateData(TRUE);
...
void CBiomini_DemoDlg::OnSelchangeType() 
{
	int nType = m_nType;
	UpdateData(TRUE);
...
void CBiomini_DemoDlg::OnBnClickedEnDetectFakeAdvanced()
{
	HUFScanner hScanner;
	UpdateData();

	if (GetCurrentScannerHandle(&hScanner)) {
		BOOL bDetectFakeEx = m_ctl_enableDetectFakeEx ? UFS_LFD_TYPE_ADVANCED : UFS_LFD_TYPE_DEFAULT;
		int ufs_res = UFS_SetParameter(hScanner, UFS_PARAM_LFD_TYPE, &bDetectFakeEx);
		if(ufs_res == UFS_OK) {
			AddMessage("Setting UFS_PARAM_LFD_TYPE = %d successful.\r\n", bDetectFakeEx);
		}
		else {
			char errStr[256];
			UFS_GetErrorString(ufs_res, errStr);
			AddMessage("Setting UFS_PARAM_LFD_TYPE failed : %s \r\n", errStr);
		}
	}
}
